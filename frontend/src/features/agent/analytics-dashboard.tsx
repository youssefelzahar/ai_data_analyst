...
export default function AnalyticsDashboard() {
  ...
  async function handleRenameConversation(
    ...
    if (!trimmed) return;
    try {
      await renameConversation(targetSessionId, trimmed);
      if (targetSessionId === sessionId && activeConversation) {
        setActiveConversation({ ...activeConversation, title: trimmed });
      }
  ...
  async function handleDeleteConversation(targetSessionId: string) {
    if (!window.confirm("Delete this conversation? This cannot be undone.")) return;
    try {
      await deleteConversation(targetSessionId);
      if (targetSessionId === sessionId) startNewConversation();
      await refreshConversations();
    } catch (error) {
      setErrorMessage(
                ...
                {conversationSummaries.map((conversation) => (
                  ...
                  >
                    ...
                    <div className="mt-2 flex gap-3 text-xs">
                      ...
                      <button
                        onClick={() =>
                          void handleDeleteConversation(conversation.session_id)
                        }

...
import DataProfile from "@/features/dataset/data-profile";
import { getDataSource } from "@/services/data-sources";
import type { DataSource } from "@/types/data-source";
import { AuthGuard } from "@/components/auth-guard";

export default function DataProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    ...
    <main className="mx-auto max-w-6xl p-8">
      ...
      <h1 className="mt-2 text-3xl font-bold tracking-tight">
        {dataSource?.name ?? "Data profile"}
      </h1>
      <p className="mt-1 text-slate-400">
        Structure, quality, and statistical summary of this dataset.
      </p>
      ...
      {dataSource && (
        <div className="mt-6">
          <DataProfile dataSourceId={dataSourceId} sourceType={dataSource.source_type} />
        </div>
      )}
    </main>
    </AuthGuard>
